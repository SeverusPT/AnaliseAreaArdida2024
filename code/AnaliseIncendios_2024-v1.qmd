...
completamente alheias aos autores.     
...
-  **Rede Nacional de Áreas Protegidas (RNAP)**: [link geral](https://geocatalogo.icnf.pt/catalogo.html) \| [link dados](https://si.icnf.pt/shp/rnap)
...
-   **Zonas Especiais de Conservação / Sítios da Diretiva Habitats (ZEC/SIC) - RN2000**: [link geral](https://geocatalogo.icnf.pt/catalogo.html) \| [link dados](https://si.icnf.pt/shp/sic)
...
geoespaciais. As ferramentas empregadas para este fim incluíram uma combinação de bibliotecas especializadas no tratamento de dados vetoriais e raster, assim como para a geração de gráficos e tabelas.
...
if(!USE_BA_INTERSECT_CACHE){
  ...
  rnap <- read_sf("../data/vector/rnap/rnap.shp") %>% 
    st_transform(crs="EPSG:32629") %>% 
...
de Strahler [^1], foram quantificados os seguintes impactos em termos de comprimento total por tipo de rio (Strahler 1 a 6):       
...
foram potencialmente afetados pelos incêndios de 2024 (i.e., como indivíduos residentes à data de 2021 dentro 
ou nas imediações dos perímetros ardidos, com níveis muito diferenciados de impacto e que os dados a que 
tivemos acesso não permitem estimar). Segundo este conjunto de dados também, um total de **`{r} scales::comma(sum(ba_bgri$N_ALOJAMENTOS_TOTAL))` alojamentos** poderão ter sido impactados (com graus de 
afetação muito diferenciados e que os dados não permitem caracterizar) estando localizados dentro 
...
[^1]: A ordem de Strahler classifica rios com base na hierarquia dos seus afluentes: rios 
...
kable(wf_counts,
      ...
      col.names = c("Classe","Nr. ocorrências", "Percentagem 2024", "Nr. ocorrências após 15 set", "Percentagem classe")) %>%
...
# # Define the breaks for the wildfire size classes
# breaks_ <- c(10, 100, 250, 500, 1000, 2500, 5000, 10000, Inf)
# 
# # Labels for each size class
# labels_ <- c("0-10 ha", "10-100 ha", "100-250 ha", 
#             "250-500 ha", "500-1000 ha", 
#             "1000-2500 ha", "2500-5000 ha",
#             "5000-10000 ha", ">10000 ha")
# # Plot the histogram with custom bins
# ggplot(effis_data_2024, aes(x = area_ht)) +
#   geom_histogram(breaks = breaks_, 
#                  fill = "lightblue", 
#                  color = "black", 
#                  closed = "left") +
#   scale_x_continuous(breaks = breaks_, labels = labels_) +
#   labs(title = "Histograma da dimensão dos incêndios (2024)",
#        x = "Classe de área (ha)", 
#        y = "Frequência") +
...
g_annual_totals <- ggplot(annual_totals, aes(x = year, y = ba_total_yr)) +
  ...
  labs(title = "Área ardida em Portugal continental 2001 - 2024",
       caption = paste0("Dados: Copernicus/EFFIS - European Forest Fire Information System (data: ",format(Sys.Date(), "%d/%m/%Y"),")\nAutor: J. Gonçalves"))
...
ba_summ_cos18$COS18n4_L <- factor(ba_summ_cos18$COS18n4_L,
  ...
  # left_join(effis_sp_2024_utm, by = "id") %>%
  # mutate(ba_perc = (ba_total_ha / area_ht)*100)
  #
...
# ba_mun_2024 <- st_intersection(conc, effis_sp_2024_utm) %>%
#   mutate(ba_ha = as.numeric(st_area(.) / 10000)) %>%
#   st_drop_geometry() %>%
#   group_by(Municipio) %>%
#   summarize(ba_total_ha = sum(ba_ha)) %>%
...
ggplot(data = ba_conc) +
  ...
  labs(title = "Área ardida por município (2024)",
       caption = paste0(
         "Dados: Copernicus/EFFIS - European Forest Fire Information System (data: ",
...
# 
# ba_rnap <- st_intersection(rnap, effis_sp_2024_utm) %>%
#   mutate(ba_ha = as.numeric(st_area(.) / 10000)) %>%
#   st_drop_geometry() %>%
#   group_by(nome_ap) %>%
#   summarize(ba_total_ha = sum(ba_ha)) %>%
...
# ba_rn2k_sic <- st_intersection(sic, effis_sp_2024_utm) %>%
#   mutate(ba_ha = as.numeric(st_area(.) / 10000)) %>%
#   st_drop_geometry() %>%
#   group_by(site_code, site_name) %>%
#   summarize(ba_total_ha = sum(ba_ha)) %>%
...
# ba_rn2k_zpe <- st_intersection(zpe, effis_sp_2024_utm) %>%
#   mutate(ba_ha = as.numeric(st_area(.) / 10000)) %>%
#   st_drop_geometry() %>%
#   group_by(site_code, site_name) %>%
#   summarize(ba_total_ha = sum(ba_ha)) %>%
